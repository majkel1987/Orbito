...
public class UpgradeSubscriptionCommandHandler : IRequestHandler<UpgradeSubscriptionCommand, Result<SubscriptionDto>>
    {
        ...
        {
            _logger.LogInformation("Upgrading subscription {SubscriptionId} to plan {NewPlanId} for client {ClientId}",
                request.SubscriptionId, request.NewPlanId, request.ClientId);

            // SECURITY: Verify tenant context
            if (!_tenantContext.HasTenant)
            {
                _logger.LogWarning("Attempted to upgrade subscription without tenant context");
            ...
            if (!await _subscriptionService.CanUpgradeAsync(subscription, request.NewPlanId, cancellationToken))
            {
                ...
                return Result.Failure<SubscriptionDto>(DomainErrors.Subscription.CannotUpgrade);
            }

...
{
    /// <summary>
    /// Validator for BulkRetryPaymentsCommand
    /// </summary>
    public class BulkRetryPaymentsCommandValidator : AbstractValidator<BulkRetryPaymentsCommand>
    {
            ...
            IPaymentRetryRepository retryRepository)
        {
            ...
            RuleFor(x => x.PaymentIds)
                .NotEmpty()
                .WithMessage("Payment IDs list cannot be empty");

            RuleFor(x => x.PaymentIds)
                .Must(ids => ids.Count <= MaxBulkRetryLimit)
                .WithMessage($"Maximum {MaxBulkRetryLimit} payments allowed per bulk retry operation");
            ...
            RuleFor(x => x.PaymentIds)
                ...
                .WithMessage("Payment IDs must be unique");
            ...
            RuleFor(x => x.Reason)
                .Must(reason => string.IsNullOrWhiteSpace(reason) || reason.Trim().Length > 0)
            ...
            RuleFor(x => x)
                .MustAsync(async (cmd, ct) =>
                {
                    ...
                    var activeRetries = existingRetries.Where(r =>
                        r.Status == Domain.Enums.RetryStatus.Scheduled ||
                        r.Status == Domain.Enums.RetryStatus.InProgress).ToList();

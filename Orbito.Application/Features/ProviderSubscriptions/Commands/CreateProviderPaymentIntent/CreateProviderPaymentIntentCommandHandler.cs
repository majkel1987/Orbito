    ...
    : IRequestHandler<CreateProviderPaymentIntentCommand, Result<CreateProviderPaymentIntentResponse>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrEmpty(provider.StripeCustomerId))
        {
            ...
            var setStripeResult = provider.SetStripeCustomerId(createCustomerResult.ExternalCustomerId!);
            if (setStripeResult.IsFailure)
            {
                return Result.Failure<CreateProviderPaymentIntentResponse>(setStripeResult.Error);
            }

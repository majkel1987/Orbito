    ...
    public class GetScheduledRetriesQueryHandler : IRequestHandler<GetScheduledRetriesQuery, PaginatedList<RetryScheduleDto>>
    {
        ...
        public async Task<PaginatedList<RetryScheduleDto>> Handle(GetScheduledRetriesQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                _logger.LogInformation("Getting scheduled retries with filters: ClientId={ClientId}, Status={Status}, Page={Page}, PageSize={PageSize}",
                    request.ClientId, request.Status, request.Pagination.PageNumber, request.Pagination.PageSize);
                ...
                if (!clientId.HasValue)
                {
                    ...
                    return PaginatedList<RetryScheduleDto>.Empty(request.Pagination.PageNumber, request.Pagination.PageSize);
                }
                ...
                return new PaginatedList<RetryScheduleDto>(items, totalCount, request.Pagination.PageNumber, request.Pagination.PageSize);
            }
            catch (OperationCanceledException)
            {
                _logger.LogWarning("Request cancelled while getting scheduled retries");
                throw;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting scheduled retries for client {ClientId}, status {Status}",

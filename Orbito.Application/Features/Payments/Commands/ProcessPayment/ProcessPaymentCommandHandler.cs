    ...
    public class ProcessPaymentCommandHandler : IRequestHandler<ProcessPaymentCommand, ProcessPaymentResult>
    {
        ...
        public async Task<ProcessPaymentResult> Handle(ProcessPaymentCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!_tenantContext.HasTenant)
            {
                ...
                return ProcessPaymentResult.FailureResult("Tenant context is required");
            }
            ...
            if (!IsValidCurrencyCode(request.Currency))
            {
                ...
                return ProcessPaymentResult.FailureResult("Invalid currency code. Must be a supported currency");
            }
            ...
            catch (ArgumentException ex)
            {
                ...
                return ProcessPaymentResult.FailureResult($"Invalid amount or currency: {ex.Message}");
            }
            ...
            try
            {
                ...
                if (client == null)
                {
                    ...
                    return await FailWithRollback("Client not found", cancellationToken);
                }
                ...
                if (client.TenantId != tenantId)
                {
                    ...
                    return await FailWithRollback("Client does not belong to the current tenant", cancellationToken);
                }
                ...
                if (subscription == null)
                {
                    ...
                    return await FailWithRollback("Subscription not found", cancellationToken);
                }
                ...
                if (subscription.ClientId != request.ClientId)
                {
                    _logger.LogWarning("Payment processing failed: Subscription {SubscriptionId} does not belong to client {ClientId}",
                        request.SubscriptionId, request.ClientId);
                    return await FailWithRollback("Subscription does not belong to the specified client", cancellationToken);
                }
                ...
                if (subscription.TenantId != tenantId)
                {
                    ...
                    return await FailWithRollback("Subscription does not belong to the current tenant", cancellationToken);
                }
                ...
                if (subscription.Status != SubscriptionStatus.Active)
                {
                    ...
                    return await FailWithRollback($"Subscription is not active. Current status: {subscription.Status}", cancellationToken);
                }
                ...
                if (subscription.Plan.Price.Currency != request.Currency)
                {
                    ...
                    return await FailWithRollback("Payment currency must match subscription currency", cancellationToken);
                }
                ...
                if (subscription.Plan.Price.Amount != request.Amount)
                {
                    ...
                    return await FailWithRollback("Payment amount must match subscription plan amount", cancellationToken);
                }
                ...
                if (!string.IsNullOrEmpty(request.ExternalTransactionId))
                {
                    ...
                    if (existingPayment != null)
                    {
                        ...
                        return ProcessPaymentResult.SuccessResult(MapToDto(existingPayment));
                    }
                }
                ...
                return ProcessPaymentResult.SuccessResult(paymentDto);
            }
            catch (DbUpdateException ex) when (IsDuplicateKeyException(ex))
            {
                _logger.LogWarning("Duplicate external transaction ID detected during save");
                await _unitOfWork.RollbackTransactionAsync(cancellationToken);
                return ProcessPaymentResult.FailureResult("Payment with this external transaction ID already exists");
            }
            catch (ArgumentException ex)
            {
                _logger.LogError(ex, "Payment processing failed due to validation error: {Message}", ex.Message);
                await _unitOfWork.RollbackTransactionAsync(cancellationToken);
                return ProcessPaymentResult.FailureResult($"Validation error: {ex.Message}");
            }
            catch (InvalidOperationException ex)
            {
                _logger.LogError(ex, "Payment processing failed due to business rule violation: {Message}", ex.Message);
                await _unitOfWork.RollbackTransactionAsync(cancellationToken);
                return ProcessPaymentResult.FailureResult($"Business rule violation: {ex.Message}");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Payment processing failed due to unexpected error: {Message}", ex.Message);
                await _unitOfWork.RollbackTransactionAsync(cancellationToken);
                return ProcessPaymentResult.FailureResult($"An unexpected error occurred while processing payment: {ex.Message}");
            }
            finally
            {
        ...
        private async Task<ProcessPaymentResult> FailWithRollback(string message, CancellationToken cancellationToken)
        {
            await _unitOfWork.RollbackTransactionAsync(cancellationToken);
            return ProcessPaymentResult.FailureResult(message);
        }

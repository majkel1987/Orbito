...
public class ProcessWebhookEventCommandHandler : IRequestHandler<ProcessWebhookEventCommand, Result>
{
    ...
    public async Task<Result> Handle(ProcessWebhookEventCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                _logger.LogError("Failed to process webhook event {EventId}: {Error}", request.EventId, processResult.Error.Message);
            ...
            {
                _logger.LogWarning("Failed to mark webhook event {EventId} as processed: {Error}", request.EventId, markResult.Error.Message);
            }
    ...
    {
        var validProviders = new[] { PaymentProviders.Stripe, PaymentProviders.PayPal, PaymentProviders.Square };
